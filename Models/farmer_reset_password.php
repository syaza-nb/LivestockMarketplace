...
if (isset($_GET['token']) && !empty(trim($_GET['token']))) {
    ...
    $sql = "SELECT * FROM farmer WHERE TRIM(reset_token_hash) = :hash LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $token_valid) {
    ...
    if ($new_password !== $confirm_password) {
        ...
        header("Location: farmer_reset_password.php?token=" . urlencode($_GET['token']));
        exit();
    }
    
    $hashed_password = password_hash($new_password, PASSWORD_BCRYPT);
    
    // Clear out reset field records upon completion
    $sql = "UPDATE farmer 
    ...
    if ($execution) {
        $_SESSION['status'] = "Farmer password updated successfully!";
...
<head>
<meta charset="UTF-8">
<title>Reset Farmer Password</title>
...
<div class="login-container">
    ...
    <div class="login-right">
        <?php if ($token_valid): ?>
            ...
            <form action="farmer_reset_password.php?token=<?= htmlspecialchars($incoming_token) ?>" method="POST" id="resetForm">
        ...
        <?php else: ?>
            <div class="error-box">
                ...
                <a href="farmer_forgot_password.php" class="btn-login-vintage" style="text-decoration: none; text-align: center; display: block; margin-top: 25px;">Request a New Link</a>
            </div>
        <?php endif; ?>
    </div>
</div>
